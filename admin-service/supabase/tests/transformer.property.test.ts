    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          expires_at: fc.option(fc.integer({ min: 0, max: 4102444800000 }).map(t => new Date(t).toISOString())),
          max_clicks: fc.option(fc.integer({ min: 1 })),
          created_at: fc.option(fc.integer({ min: 0, max: 4102444800000 }).map(t => new Date(t).toISOString())),
          updated_at: fc.option(fc.integer({ min: 0, max: 4102444800000 }).map(t => new Date(t).toISOString())),
